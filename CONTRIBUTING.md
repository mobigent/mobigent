...
Mobigent is early, energetic, and wide open for builders. The most valuable contributions are small, tested improvements to the SDK shape, gateway behavior, docs, and examples.
...
- Run `npm run verify`; CI runs the same gate on Node 20 and Node 22.
- Run `npm run demo`.
...
GitHub Actions runs `npm ci` and `npm run verify` on pull requests and pushes to `main`. The verification gate includes SDK typechecks, tests, package packing checks, and the Vite production build.

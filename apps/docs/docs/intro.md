...
The app exposes normal functions such as `expense.list` and `expense.create`. The backend waits for the app, so server code can use `mobigent.use<MyAppFunctions>()` and call `app.expense.create(input)` like ordinary backend code without loading mobile runtime code. Mobigent handles connection setup, validation, approval, retries, agent setup, and audit events.

...
console.log(`Loaded ${runtime.rawTools.length} mobile tool${runtime.rawTools.length === 1 ? "" : "s"}.`);
...
if (process.env.MOBIGENT_WATCH_TOOLS === "true") {
  ...
  for await (const update of watchMobigentProviderRuntime({
    ...
    console.log(`${update.reason}: ${update.rawTools.length} tool${update.rawTools.length === 1 ? "" : "s"}`);
  }
}

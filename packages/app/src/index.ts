...
export type MobigentAppPackageIdentityOptions = Omit<
...
export type MobigentAppPackageInput =
  MobigentAppPackageOptions | MobigentSimpleFunctionMap | string;
...
type MobigentAppConnectSettings =
  MobigentSimpleConnectionSettings | MobigentBackendConnectionTarget;
...
export function withMobigent<P extends object>(
  ...
  input: MobigentAppPackageInput,
  functionsOrOptions?:
    | MobigentSimpleFunctionMap
    | MobigentAppPackageIdentityOptions
    | Omit<AgentAppRootProps, 'children'>,
  options: MobigentAppPackageIdentityOptions = {},
): ComponentType<P> {
  ...
  return withMobigentRoot(
    App,
    input as MobigentSimpleAppInput,
    functionsOrOptions as Omit<AgentAppRootProps, 'children'> | undefined,
  );
}
...
function normalizeBackendBackedAppInput(
  input: MobigentAppPackageOptions,
): MobigentAppPackageOptions {
  ...
  const options = input as MobigentWithAppOptions & {
    functions?: MobigentSimpleFunctionMap;
    backend?: MobigentAppBackendSource;
  };
...
function isFunctionMapInput(value: MobigentAppPackageInput): value is MobigentSimpleFunctionMap {
  if (
    ...
    typeof value !== 'object' ||
    Array.isArray(value) ||
    isFeatureInput(value as MobigentAppPackageOptions)
  ) {
    return false;
  }
...
function isIdentityOptions(
  value: MobigentSimpleFunctionMap | MobigentAppPackageIdentityOptions,
): value is MobigentAppPackageIdentityOptions {
...
function resolvePackageFeatures(
  input: MobigentAppPackageOptions,
): MobigentSimpleFeature | MobigentSimpleFeature[] {
  if (isFeatureInput(input)) {
    return input;
  }
...
function resolvePackageConnectionSettings(
  input: MobigentAppPackageOptions,
...
function resolveBackendConnectionSettings(
  target: MobigentLegacyBackendConnectionTarget,
): MobigentSimpleConnectionSettings {
  const pairing = resolveAppPairing(target.pairing);
  const config = pairing ?? target.connection ?? target.defaultApp ?? target;

  return {
    appId: config.appId,
    appName: config.appName,
    pairing,
    connection: (config as { connection?: MobigentSimpleBackendConnection }).connection,
...
function resolveAppBackendSettings(
  backend: MobigentAppBackendSource | undefined,
): MobigentSimpleConnectionSettings {
  if (!backend) {
    return {};
  }
...
function resolveAppPairing(
  pairing: MobigentAppPairingSource | undefined,
): MobigentSimpleAppConfig | undefined {
  return typeof pairing === 'function' ? pairing() : pairing;
}

function isFeatureInput(
  value: MobigentAppPackageOptions,
...
function isFeature(
  value: MobigentAppPackageOptions | MobigentSimpleFeature,

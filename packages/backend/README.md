...
For quick one-off explicit calls, use `mobigent.call("expense.create", input)`. The older `mobigent.functions.expense.create(input)` shape is still available for compatibility. For shutdown, call `await mobigent.stop()`.

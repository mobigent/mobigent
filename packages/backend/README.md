...
For quick one-off explicit calls, use `mobigent.call("expense.create", input)`. For reusable aliases, use `mobigent.fn("expense.create")` or `mobigent.feature("expense")`. For shutdown, call `await mobigent.stop()`.

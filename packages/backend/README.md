...
For quick one-off explicit calls, use `mobigent.callApp("expense.create", input)`. For reusable aliases, `mobigent.feature("expense")` and `mobigent.function("expense.create")` are still available. For shutdown, call `await mobigent.stop()`.

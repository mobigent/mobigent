...
export function validateCapabilityManifest(manifest: unknown): ValidationResult {
  ...
  if (protocolVersion !== undefined && (!Number.isInteger(protocolVersion) || Number(protocolVersion) < 1)) {
...
function validateUniqueManifestToolNames(manifest: Record<string, unknown>, errors: string[]) {
  ...
  for (const candidate of candidates) {
    ...
    if (previousPath) {
      errors.push(`${candidate.path} exposes duplicate tool name ${fullToolName}; first declared at ${previousPath}`);
      continue;
    }
...
function validateResourceDefinition(resource: Record<string, unknown>, path: string, errors: string[]) {
...
function validateComponentDefinition(component: Record<string, unknown>, path: string, errors: string[]) {
...
function validateConfirmationPolicy(policy: unknown, path: string, errors: string[]) {
  ...
  if (policy.risk !== undefined && policy.risk !== "low" && policy.risk !== "medium" && policy.risk !== "high") {
...
function matchesSchemaType(type: string | string[], value: unknown) {
  return Array.isArray(type) ? type.some((candidate) => matchesType(candidate, value)) : matchesType(type, value);
}

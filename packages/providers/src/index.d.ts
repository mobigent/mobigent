export type ProviderKind = "mcp-stdio" | "openapi" | "chatgpt-actions" | "openai-responses" | "anthropic-tool-use" | "google-gemini" | "aws-bedrock-converse" | "vercel-ai-sdk" | "langchain" | "llamaindex" | "mastra" | "claude-desktop" | "cursor" | "vscode" | "generic-agent";
...
export type MobigentHttpClient = {
    ...
    callTool: (toolName: string, input?: Record<string, unknown>, options?: MobigentToolCallOptions) => Promise<unknown>;
    watchTools: (options?: MobigentToolStreamOptions) => AsyncIterable<MobigentToolChangeEvent>;
    headers: () => Record<string, string>;
};
export type MobigentHttpErrorCode = "unauthorized" | "forbidden" | "invalid_input" | "not_found" | "rate_limited" | "gateway_error" | "network_error" | "invalid_response";
...
export declare function createOpenAiResponsesProvider(options: HttpAgentOptions): ProviderDescriptor;
export declare function createAnthropicToolUseProvider(options: HttpAgentOptions): ProviderDescriptor;
export declare function createGoogleGeminiProvider(options: HttpAgentOptions): ProviderDescriptor;
export declare function createAwsBedrockConverseProvider(options: HttpAgentOptions): ProviderDescriptor;
...
export declare function createMobigentHttpClient(options: MobigentHttpClientOptions): MobigentHttpClient;
...
export declare function toGeminiFunctionDeclarations(tools: MobigentHttpTool[]): GeminiFunctionDeclaration[];
export declare function toBedrockToolConfigTools(tools: MobigentHttpTool[]): BedrockToolSpecification[];
export declare function createMobigentToolExecutor(client: MobigentHttpClient): (toolName: string, input?: Record<string, unknown>) => Promise<unknown>;
export declare function toExecutableTools(tools: MobigentHttpTool[], client: MobigentHttpClient): MobigentExecutableTool[];
export declare function toLangChainTools(tools: MobigentHttpTool[], client: MobigentHttpClient): LangChainToolDefinition[];
export declare function toLlamaIndexTools(tools: MobigentHttpTool[], client: MobigentHttpClient): LlamaIndexToolDefinition[];
export declare function toMastraTools(tools: MobigentHttpTool[], client: MobigentHttpClient): MastraToolDefinition[];
export declare function toVercelAiSdkTools(tools: MobigentHttpTool[], client: MobigentHttpClient): Record<string, VercelAiSdkToolDefinition>;

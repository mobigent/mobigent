...
console.log(
  `auth: ${hasToken ? 'token present' : loggedInUser ? `logged in as ${loggedInUser}` : 'not logged in'}`,
);
...
if (!hasToken && !trustedPublishing && !loggedInUser) {
  failures += 1;
  console.log(
    'fail auth: run npm login, set NODE_AUTH_TOKEN/NPM_TOKEN, or configure npm Trusted Publishing.',
  );
}
...
for (const group of packageGroups) {
  ...
  for (const packageName of group.packages) {
    ...
    if (!repository?.includes(expectedRepository)) {
      failures += 1;
      console.log(
        `  fail ${packageName}: repository is ${repository ?? 'missing'}, expected ${expectedRepository}`,
      );
      continue;
    }

    console.log(
      `  ok   ${packageName}@${view.version ?? 'unknown'}: published with expected repository`,
    );
  }
...
if (trustedPublishing && missing > 0 && !hasToken && !loggedInUser) {
  ...
  console.log('');
  console.log(
    'fail trusted publishing: npm requires the packages to exist before Trusted Publishing can publish them.',
  );
  console.log(
    'Use npm login or NPM_TOKEN for the first publish, then enable Trusted Publishing for future releases.',
  );
}
...
if (missing > 0 && (hasToken || loggedInUser)) {
  console.log('');
  console.log(
    `${missing} package(s) are ready for first publish from this authenticated environment.`,
  );
}

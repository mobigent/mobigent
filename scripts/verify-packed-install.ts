...
assert.equal(typeof mobigent.app("expense").create, "function");
assert.equal(typeof mobigent.app("expense", { createExpense: "create" }).createExpense, "function");
assert.equal(typeof mobigent.app("expense", ["create"]).create, "function");
...
`,
  ...
  assert.match(
    await run(join(binDir, 'mobigent'), ['backend', '--help'], appDir),
    /mobigent-backend/,
  );
  assert.match(
    await run(join(binDir, 'mobigent'), ['install', 'backend', '--dry-run'], appDir),
    /npm install @mobigent\/backend/,
  );
  assert.match(
    await run(join(binDir, 'mobigent'), ['install', 'app', '--dry-run'], appDir),
    /npm install @mobigent\/app/,
  );
  assert.match(
    await run(join(binDir, 'mobigent'), ['new', '--help'], appDir),
    /create-mobigent-app/,
  );
  assert.match(
    await run(join(binDir, 'create-mobigent-app'), ['--help'], appDir),
    /create-mobigent-app/,
  );
  assert.match(
    await run(join(binDir, 'mobigent-install'), ['backend', '--dry-run'], appDir),

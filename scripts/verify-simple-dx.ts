...
const appBackendTargetType =
  appPackageRoot.match(/type MobigentBackendConnectionTarget = \{[\s\S]*?^};/m)?.[0] ?? '';
const backendPublicType =
  backendPackageRoot.match(/export type MobigentBackend = \{[\s\S]*?^};/m)?.[0] ?? '';
...
assert.doesNotMatch(
  appPackageRoot,
  /export const setup\b/,
  '@mobigent/app root should not expose setup aliases',
);
assert.doesNotMatch(
  appPackageRoot,
  /export const connect\b/,
  '@mobigent/app root should not expose top-level connection plumbing; use createApp(functions).connect() for non-React hosts',
);
...
for (const typeName of [
  ...
  assert.match(
    ...
    `@mobigent/app should expose friendly ${typeName} type`,
  );
}
assert.match(
  backendPackageRoot,
  /connection: MobigentBackendClient/,
  '@mobigent/backend should expose a clean backend.connection pairing object',
);
assert.match(
  backendPackageRoot,
  /agentUrl: string/,
  '@mobigent/backend should expose a friendly agentUrl alias',
);
assert.match(
  backendPackageRoot,
  /appConnectionUrl: string/,
  '@mobigent/backend should expose a friendly appConnectionUrl alias',
);
assert.match(
  backendPackageRoot,
  /backendUrl: string/,
  '@mobigent/backend should expose backendUrl in app handoff settings',
);
assert.doesNotMatch(
  backendPublicType,
...
assert.match(
  ...
  '@mobigent/backend should expose grouped backend.setup helpers for agent setup',
);
assert.match(
  backendPackageRoot,
  /connect: MobigentBackendSetupAccessor/,
  '@mobigent/backend should expose backend.connect helpers for agent setup',
);
assert.match(
  backendPackageRoot,
  /<const T extends MobigentBackendAppFunctionContract>\(\): MobigentBackendAppFunctions<T>/,
  '@mobigent/backend should expose backend.app<MyAppFunctions>() as the clearest typed app-call surface',
);
assert.match(
  backendPackageRoot,
  /forApp\(\): MobigentBackendClient/,
  '@mobigent/backend should keep backend.forApp() compatibility for explicit app settings',
);
assert.match(
  backendPackageRoot,
  /appSettings\(\): MobigentBackendClient/,
  '@mobigent/backend should keep backend.appSettings() compatibility',
);
assert.match(
  backendPackageRoot,
  /appClient\(\): MobigentBackendClient/,
  '@mobigent/backend should keep backend.appClient() compatibility',
);
assert.match(
  backendPackageRoot,
  /pairing\(\): MobigentBackendClient/,
  '@mobigent/backend should keep backend.pairing() compatibility',
);
assert.match(
  backendPackageRoot,
  /chatgpt\(options\?: MobigentAgentOptions\): ProviderBundle/,
  '@mobigent/backend should expose backend.chatgpt() for common ChatGPT setup',
);
assert.match(
  backendPackageRoot,
  /claude\(options\?: MobigentAgentOptions\): ProviderBundle/,
  '@mobigent/backend should expose backend.claude() for common Claude setup',
);
assert.match(
  backendPackageRoot,
  /openai\(options\?: MobigentAgentOptions\): ProviderBundle/,
  '@mobigent/backend should expose backend.openai() for common OpenAI setup',
);
for (const typeName of [
  'Backend',
  'BackendOptions',
  ...
  assert.match(
    backendPackageRoot,
    new RegExp(`export type ${typeName}\\b`),
    `@mobigent/backend should expose friendly ${typeName} type`,
  );
}
assert.match(
  appPackageRoot,
  /backend\?: MobigentAppBackendSource/,
  '@mobigent/app should accept a backend object as the clean app handoff',
);
assert.doesNotMatch(
  appPackageRoot,
  /export type (AppConnectionSettings|BackendConnection|MobigentAppBackendSource|MobigentBackendConnectionTarget|MobigentAppConnectSettings)\b/,
  '@mobigent/app root should not expose backend handoff or connection settings as beginner types',
);
assert.doesNotMatch(
  appPackageRoot,
  /export \{[\s\S]*type MobigentSimple(BackendConnection|Connection|ConnectionOptions|ConnectionSettings)[\s\S]*\} from "@mobigent\/react-native\/app"/,
  '@mobigent/app root should not re-export low-level connection types from the simple import path',
);
assert.match(
  appPackageRoot,
  /normalizeBackendBackedAppInput/,
  '@mobigent/app should infer app identity and connection from a backend object',
);
assert.match(
  appPackageRoot,
  /appId: options\.appId \?\? backendSettings\.appId/,
  '@mobigent/app should use backend.appId when app code does not repeat one',
);
assert.match(
  appBackendTargetType,
  /pairing\?: MobigentAppPairingSource/,
  '@mobigent/app should keep older backend pairing compatibility internally',
);
assert.doesNotMatch(
  appPackageRoot,
  /export type (Pairing|AppPairing|BackendPairing|MobigentAppPairing|MobigentAppPairingSource)\b/,
  '@mobigent/app root should not expose pairing jargon as public beginner types',
);
assert.match(
  appPackageRoot,
  /MobigentSimpleAppConfig \| \(\(\) => MobigentSimpleAppConfig\)/,
  '@mobigent/app should still accept older pairing objects internally',
);
assert.match(
  appBackendTargetType,
  /connection\?: MobigentSimpleConnectionSettings/,
  '@mobigent/app should keep backend.connection compatibility',
);
assert.doesNotMatch(
  appBackendTargetType,
  /urls|defaultApp|gatewayUrl/,
  '@mobigent/app internal backend target type should not advertise legacy backend internals',
);
assert.match(
  backendPublicType,
  /connection: MobigentBackendClient/,
  '@mobigent/backend public type should expose backend.connection',
);
assert.doesNotMatch(
  backendPublicType,
...
assert.match(
  mobigentPackageRoot,
  /startMobigent/,
  'mobigent package root should re-export backend SDK helpers for tiny demos',
);
assert.match(
  mobigentPackageRoot,
  /write/,
  'mobigent package root should re-export common function metadata helpers',
);
assert.doesNotMatch(
  mobigentPackageRoot,
  /createApp|withMobigent/,
  'mobigent package root should not pull React-facing app setup into backend-only imports',
);
assert.doesNotMatch(
  mobigentPackageRoot,
...
for (const [path, packageJson] of packageJsons) {
  assert.match(
    ...
    /app functions|backend SDK|starter|CLI/i,
    `${path} should describe the simple SDK model`,
  );
...
const backendWithAppDirFile =
  backendWithAppDir.find((file) => file.path === 'src/mobigent.ts')?.contents ?? '';
...
const rnFeature =
  rnFiles.find((file) => file.path === 'src/mobigent-functions/expense.ts')?.contents ?? '';
...
assert.doesNotMatch(
  rnFeature,
  /defineFeature|defineMobigentAction|createAgentModule|registerAction/,
);
...
assert.doesNotMatch(
  starterCapabilities,
  /defineFeature|defineMobigent|import \{ defineFeature, mobigent \}/,
);
...
for (const path of [
  ...
  assert.match(
    contents,
    /createApp\(\{|createApp\(functions|createApp\(appFunctions/,
    `${path} should teach the no-config app path`,
  );
  assert.match(
    contents,
  ...
  assert.match(
    contents,
    /startMobigent\(\)/,
    `${path} should teach the no-config backend start path`,
  );
  assert.doesNotMatch(
    contents,
  ...
  assert.doesNotMatch(
    contents,
    /npx mobigent-init/,
    `${path} should not teach the legacy app init binary`,
  );
  assert.doesNotMatch(
    contents,
...
for (const path of [
  ...
  assert.match(
    ...
    `${path} should answer the old init-command confusion directly`,
  );
  assert.match(
    contents,
    /mobigent-rn-init --feature-only/,
    `${path} should frame the React Native CLI as optional sample-module generation only`,
  );
  assert.doesNotMatch(
    contents,
    /mobigent-init/,
    `${path} should not preserve the broad legacy init command name`,
  );
  assert.doesNotMatch(
    contents,
    /mobigent-install|npm exec --yes|create-mobigent-app-0\.1\.15\.tgz/,
    `${path} should not show preview installer commands in the existing-app path`,
  );
  assert.match(
    contents,
    /npm install @mobigent\/app[\s\S]{0,120}?npm install @mobigent\/backend/,
    `${path} should lead with normal package installs`,
  );
  assert.match(
    contents,
    /appFunctions[\s\S]{0,240}?createApp\(appFunctions\)|createApp\(\{/,
    `${path} should teach createApp(functions)`,
  );
  assert.match(
    contents,
    /withMobigent\(App, \{/,
    `${path} should teach the one-file existing-app wrapper`,
  );
  assert.match(
    contents,
    /startMobigent\(\)/,
  ...
  assert.match(
    contents,
    /EXPO_PUBLIC_MOBIGENT_APP[\s\S]{0,240}?MOBIGENT_APP|MOBIGENT_APP[\s\S]{0,240}?EXPO_PUBLIC_MOBIGENT_APP/,
    `${path} should teach matching short env identity for production`,
  );
  assert.match(
    contents,
    /mobigent\.app<MyAppFunctions>\(\)[\s\S]{0,160}?app\.expense\.create/,
    `${path} should teach backend calls through app-owned functions`,
  );
  assert.match(
    contents,
    /Most apps do not need that on day one/,
    `${path} should keep backend-friendly aliases optional`,
  );
  assert.match(
    contents,
    /What Developers Should Care About/,
    `${path} should separate developer-owned concerns from SDK-owned plumbing`,
  );
  assert.match(contents, /What Mobigent Handles/, `${path} should make SDK-owned work explicit`);
  assert.doesNotMatch(
    contents,
    /defineMobigentConfig|mobigent\.app\.json|appDir\b|backend\.defaultApp|backend\.appSettings\(\)|backend\.pairing\(\)/,
    `${path} should not pull generated config or legacy backend handoff into existing-app adoption`,
  );
}
...
for (const path of ['packages/app/README.md', 'packages/react-native/README.md']) {
  ...
  assert.match(
    contents,
    /createApp\(appFunctions\)|createApp\(\{/,
    `${path} should teach createApp(functions) first`,
  );
  assert.match(
    contents,
  ...
  assert.doesNotMatch(
    contents,
    /npx mobigent-init|mobigent\.app\.json|appDir\b/,
    `${path} should not teach app init or generated app config`,
  );
  assert.doesNotMatch(
    contents,
    /mobigent-install|npm exec --yes|create-mobigent-app-0\.1\.15\.tgz/,
    `${path} should not show preview installer commands in the package README`,
  );
  assert.match(
    contents,
    /EXPO_PUBLIC_MOBIGENT_APP|MOBIGENT_APP/,
    `${path} should mention env-configured production identity`,
  );
  assert.doesNotMatch(
    contents,
...

{
  ...
  assert.match(
    appReadme,
    /type AppFunctions/,
    'packages/app/README.md should teach the friendly AppFunctions type',
  );
  assert.match(
    appReadme,
    /MobigentApp/,
    'packages/app/README.md should list the friendly MobigentApp type',
  );
  assert.match(
    appReadme,
    /createApp\(appFunctions, \{[\s\S]{0,120}?backend/,
    'packages/app/README.md should teach passing backend settings without repeating the app id',
  );
  assert.doesNotMatch(
    appReadme,
    /MobigentAppBackendSource|BackendConnection|AppConnectionSettings|MobigentAppConnectSettings|MobigentSimpleConnection/,
    'packages/app/README.md should not advertise connection or handoff type names',
  );
  assert.doesNotMatch(
    appReadme,
    /backend\.forApp\(\)/,
    'packages/app/README.md should not teach explicit handoff settings in the beginner surface',
  );
  assert.doesNotMatch(
    appReadme,
    /backend\.appSettings\(\)/,
    'packages/app/README.md should not teach appSettings as the beginner handoff',
  );
  assert.doesNotMatch(
    appReadme,
    /backend\.pairing\(\)/,
    'packages/app/README.md should not teach pairing as the beginner settings name',
  );
}

{
  ...
  assert.match(
    backendReadme,
    /type Backend/,
    'packages/backend/README.md should teach the friendly Backend type',
  );
  assert.match(
    backendReadme,
    /BackendOptions/,
    'packages/backend/README.md should list the friendly BackendOptions type',
  );
  assert.match(
    backendReadme,
    /BackendStartOptions/,
    'packages/backend/README.md should list the friendly BackendStartOptions type',
  );
  assert.match(
    backendReadme,
    /createApp\(appFunctions, \{[\s\S]{0,120}?backend/,
    'packages/backend/README.md should teach passing backend settings without repeating the app id',
  );
  assert.match(
    backendReadme,
    /mobigent\.connect\.chatgpt\(/,
    'packages/backend/README.md should teach ChatGPT setup through connect helpers',
  );
  assert.match(
    backendReadme,
    /mobigent\.connect\.claude\(/,
    'packages/backend/README.md should teach Claude setup through connect helpers',
  );
  assert.match(
    backendReadme,
    /mobigent\.connect\.openai\(/,
    'packages/backend/README.md should teach OpenAI setup through connect helpers',
  );
  assert.match(
    backendReadme,
    /remain available for compatibility/,
    'packages/backend/README.md should mention compatibility without leading with it',
  );
  assert.doesNotMatch(
    backendReadme,
    /backend\.forApp\(\)|backend\.appSettings\(\)|backend\.pairing\(\)|BackendPairing|BackendConnection/,
    'packages/backend/README.md should not promote older setup or connection names',
  );
  assert.doesNotMatch(
    backendReadme,
    /mobigent\.functions\.expense\.create/,
    'packages/backend/README.md should not show the dynamic backend call as a product example',
  );
  assert.match(backendReadme, /startMobigent\(\)/);
  assert.match(
    backendReadme,
    /import type \{ MyAppFunctions \}/,
    'packages/backend/README.md should teach type-only app function sharing',
  );
  assert.match(
    backendReadme,
    /mobigent\.app<MyAppFunctions>\(\)/,
    'packages/backend/README.md should teach typed backend calls without importing app runtime code',
  );
  assert.doesNotMatch(
    backendReadme,
...
for (const path of [
  ...
  assert.match(
    contents,
    /startMobigent/,
    `${path} should teach startMobigent as the backend entrypoint`,
  );
  assert.match(
    contents,
    /startMobigent\(\)/,
  ...
  assert.doesNotMatch(
    contents,
    /startMobigent\(appId, appName\)/,
    `${path} should not require an app name in the beginner path`,
  );
  assert.doesNotMatch(
    contents,
  ...
  assert.match(
    contents,
    /mobigent\.app<MyAppFunctions>\(\)|backend\.app<MyAppFunctions>\(\)|mobigent\.app&lt;MyAppFunctions&gt;\(\)/,
    `${path} should teach the type-only backend function path`,
  );
  assert.doesNotMatch(
    contents,
    /backend\.defaultApp|connectionUrl: backend|mobigent\.app\.json|appDir\b|npx mobigent-backend|mobigent-backend --app/,
...
for (const path of ['docs/api/README.md', 'apps/docs/docs/api.md']) {
  ...
  assert.match(
    contents,
    /createApp\(functions, \{ backend \}\)|createApp\(expenseFunctions, \{[\s\S]{0,80}?backend/,
    `${path} should teach app setup with backend settings`,
  );
  assert.match(
    contents,
    /\.\s*connect\(\)/,
    `${path} should teach no-argument connect after setup`,
  );
  assert.doesNotMatch(
    contents,
    /MobigentAppBackendSource|backend\.forApp\(\)/,
    `${path} should not advertise explicit backend handoff types or methods`,
  );
  assert.doesNotMatch(
    contents,
    /mobigent\.connect\(backend\)/,
    `${path} should not teach backend-object connect as the beginner path`,
  );
  assert.doesNotMatch(
    contents,
    /mobigent\.connect\(backend\.pairing\(\)\)/,
    `${path} should not teach connect-time pairing as the beginner path`,
  );
}
...
for (const path of [
  ...
  assert.match(
    contents,
    /backend: backend|backend,|backend\s*\n\s*\}/,
    `${path} should teach app setup with the backend object`,
  );
  assert.match(
    contents,
    /backend: backend|backend,|backend\s*\n\s*\}/,
    `${path} should teach direct backend object setup`,
  );
  assert.doesNotMatch(
    contents,
    /backend\.forApp\(\)/,
    `${path} should not teach explicit backend handoff setup`,
  );
  assert.doesNotMatch(
    contents,
    /createApp\("com\.acme\.expenses", (appFunctions|expenseFunctions), \{[\s\S]{0,80}?backend/,
    `${path} should not repeat app id when a backend object can provide it`,
  );
  assert.match(
    contents,
    /\.\s*connect\(\)/,
    `${path} should teach no-argument connect after backend setup`,
  );
  assert.doesNotMatch(
    contents,
    /backend: backend\.appSettings\(\)/,
    `${path} should not teach appSettings as the beginner handoff`,
  );
  assert.doesNotMatch(
    contents,
    /pairing: backend\.pairing\(\)/,
    `${path} should not teach pairing as the beginner setup word`,
  );
  assert.doesNotMatch(
    contents,
    /mobigent\.connect\(backend\)/,
...
for (const path of [
  ...
  assert.doesNotMatch(
    contents,
    /mobigent\.functions|functions\.expense|mobigent\.app\.expense/,
    `${path} should keep old dynamic backend surfaces out of beginner docs`,
  );
}
...
for (const path of [
  ...
  assert.match(
    contents,
    /mobigent\.app<MyAppFunctions>\(\)|mobigent\.app&lt;MyAppFunctions&gt;\(\)|app\.expense\.create/,
    `${path} should teach the typed shared-shape backend functions surface`,
  );
  assert.match(
    contents,
    /withMobigent\(App, \{/,
...
for (const path of ['docs/api/README.md', 'apps/docs/docs/api.md']) {
  ...
  assert.doesNotMatch(
    contents,
    /## Capability Types/,
    `${path} should frame beginner docs as app functions`,
  );
  assert.doesNotMatch(
    contents,
    /defineFeature|defineMobigentConfig|connectMobigent|registerFeatures/,
    `${path} should not teach lower-level app lifecycle helpers in the package API docs`,
  );
  assert.doesNotMatch(
    contents,
    /appSettings|pairing\(|BackendPairing|AppPairing|Pairing\b/,
    `${path} should not promote older setup names in package API docs`,
  );
  assert.doesNotMatch(
    contents,
    /feature\("expense"\)|functions\.expense\.create|mobigent\.functions/,
    `${path} should not show older dynamic backend API examples`,
  );
  assert.doesNotMatch(
    contents,
...

{
  ...
  assert.match(
    docsPage,
    /Existing app path/,
    'website docs should include a dedicated existing-app adoption section',
  );
  assert.match(
    docsPage,
    /No generator\. No copied config/,
    'website docs should make generator-free adoption visible',
  );
  assert.match(
    docsPage,
    /sample generator is only for runnable demos/,
    'website docs should frame generators as demo helpers',
  );
  assert.match(
    docsPage,
    /client\.functions\("expense"\)/,
    'website native examples should teach grouped native app functions',
  );
  assert.match(
    docsPage,
    /expense\.write\(|write\(\s*"create"/,
    'website native examples should teach native writes inside grouped functions',
  );
  assert.match(
    docsPage,
    /expense\.read\(|read\(\s*"list"/,
    'website native examples should teach native reads inside grouped functions',
  );
  assert.doesNotMatch(
    docsPage,
...

{
  ...
  const homeQuickstart = homePage.slice(
    homePage.indexOf('const quickstart'),
    homePage.indexOf('function App'),
  );
  assert.doesNotMatch(
    homePage,
...
for (const path of ['scripts/check-npm-status.ts', 'scripts/check-npm-publish-readiness.ts']) {
  ...
  assert.match(
    ...
    `${path} should report SDK packages separately`,
  );
  assert.match(
    contents,
    /runtime dependency packages/,
    `${path} should frame internal packages as runtime dependencies`,
  );
}
...
for (const path of [
  ...
  assert.match(
    contents,
    /@mobigent\/backend/,
    `${path} should anchor native SDKs to the backend package`,
  );
  assert.match(
    contents,
    /client\.functions\("expense"\)/,
    `${path} should teach grouped native app functions`,
  );
  assert.doesNotMatch(
    contents,
...
assert.match(
  iosGuide,
  /MobigentClient\(\s*[\s\S]{0,120}?appId: "com\.example\.expenses"[\s\S]{0,120}?appName: "Expenses"[\s\S]{0,80}?\)/,
);
...
assert.match(
  androidGuide,
  /MobigentClient\.Builder\(context\)[\s\S]{0,160}?\.appId\("com\.example\.expenses"\)[\s\S]{0,80}?\.appName\("Expenses"\)[\s\S]{0,80}?\.build\(\)/,
);
assert.match(androidGuide, /\.backendUrl\("ws:\/\/YOUR_MAC_LAN_IP:8787"\)/);
assert.match(androidGuide, /read\(\s*"list"/);
assert.doesNotMatch(
  androidGuide.slice(
    androidGuide.indexOf('## Create A Client'),
    androidGuide.indexOf('That default connects'),
  ),
  /\.gatewayUrl\(/,
...
assert.ok(
  quickstart.indexOf('## 1. Add App Functions To An Existing App') <
    quickstart.indexOf('## 5. Optional Starter'),
...
const packageSection = rootReadme.slice(
  rootReadme.indexOf('## Packages'),
  rootReadme.indexOf('## Examples'),
);
assert.match(
  packageSection,
  /@mobigent\/app[\s\S]{0,200}?@mobigent\/backend/,
  'README packages should lead with the two public SDK packages',
);
assert.doesNotMatch(
  packageSection,
  /@mobigent\/core|@mobigent\/gateway|@mobigent\/providers/,

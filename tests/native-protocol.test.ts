...
test('native iOS and Android manifests use the shared protocol shape', () => {
  for (const sdk of ['ios', 'android'] as const) {
    ...
    assert.equal(
      toolName(manifest.appId, manifest.actions[0].name),
      `com_mobigent_examples_${sdk}_expense.create`,
    );
    assert.equal(
      toolName(manifest.appId, `get_${manifest.resources[0].name}`),
      `com_mobigent_examples_${sdk}_expense.get_list`,
    );
    assert.equal(
      toolName(manifest.appId, `show_${manifest.components[0].name}`),
      `com_mobigent_examples_${sdk}_expense.show_expense_detail`,
    );
  }
});
